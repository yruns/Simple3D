...
class Model1(torch.nn.Module):
    ...
    def forward(self, xyz, sample_idx):
        ...
        xyz_features, center, ori_idx, center_idx = self.xyz_backbone(xyz, sample_idx)
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts, sample_idx):
        if self.encoder_dims != self.trans_dim:
            B, C, N = pts.shape
            pts = pts.transpose(-1, -2)  # B N 3
            # divide the point clo  ud in the same form. This is important
            neighborhood, center, ori_idx, center_idx = self.group_divider(pts, sample_idx)
            # # generate mask
