...
class PointFeatures(Features1):
    ...
    def predict(self, sample, mask, label, pcd_path, save_path_full, save_path):
        ...
        s_map = self.compute_s_s_map(xyz_patch, mask, label, center, neighbor_idx, nonzero_indices,
                                     unorganized_pc_no_zeros.contiguous(), center_idx)
    ...
    def run_coreset(self, class_name):
        ...
        torch.save(self.patch_lib, class_name + "_patch_lib.pth")

    def compute_s_s_map(self, patch, mask, label, center, neighbour_idx, nonzero_indices, xyz, center_idx,
                        nonzero_patch_indices=None):
        '''
        center: point group center position
        neighbour_idx: each group point index
        nonzero_indices: point indices of original point clouds
