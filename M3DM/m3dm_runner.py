...
class M3DM():
    ...
    def fit(self, class_name):
        train_loader = DataLoader(Dataset3dad_train(self.root_dir, class_name, 2048, True), num_workers=1, batch_size=1,
    ...
    def evaluate(self, class_name):
        ...
        image_ap = dict()
        pixel_ap = dict()
        test_loader = DataLoader(Dataset3dad_test(self.root_dir, class_name, 2048, True), num_workers=1, batch_size=1,
                                 shuffle=True, drop_last=False)
        path_list = []
        with torch.no_grad():

            for sample, mask, label, pcd_path in tqdm(test_loader,
        ...
        for method_name, method in self.methods.items():
            ...
            print(
                f'Class: {class_name}, {method_name} Object AUROC: {method.image_rocauc:.3f}, {method_name} Point AUROC: {method.pixel_rocauc:.3f}, {method_name} Object AUPR: {method.image_ap:.3f}, {method_name} Point AuPR: {method.pixel_ap:.3f}')
